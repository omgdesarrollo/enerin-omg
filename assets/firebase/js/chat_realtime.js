...
//        alert("send");
console.log("e",userRef)
//messageRef="e";
        chat_realtime(userRef, messageRef, apis, a.user, a.avatar, imageDir);
//          chat_realtime_method(userRef, messageRef, apis,a.user, a.avatar, imageDir)
    } else {
        alert("no ");
         document.getElementsByClassName('app-one')[0].style.display = "block";
...
function loginFunction(user,avatar) {
...
document.getElementsByClassName("heading-logout")[0].addEventListener("click", function() {
    ajax("POST", "data=logout", function(res) {
        ...
        if (a.status == 'success') {
            ...
            userRef.push(b);
            setTimeout(function() {
                window.location.href = "cerrando sesion";
            }, 1500);
        }
    });
});
...
var chat_realtime = function(j, k, l, m, n, imageDir) {
    ...
    userMysql(function(a) {
        console.log(a);
//        alert("entro en usermysql");
        allUser = a.all;
        chatUser = a.chat;
        allUser.forEach(function(a) {
            if (a.name != m) {
//                alert("entro en este lado");
                sideTwoHTML(a);
            }
        });
